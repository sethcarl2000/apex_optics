...
int metropolis_A_production(const bool is_RHRS)
{
                            ...
                            ](MetPhasePoint_t& point) 
    {
        ...
        TVector3 p_electron( Gaus(), Gaus(), Gaus() );
    ...
    while (n_generated < n_stats) {
        ...
        if (!phase_point.in_acceptance) { 
            if (++i_start > max_starting_attempts) break; 
            continue; 
        }

        x_and_theta->Fill(phase_point.x, phase_point.theta*1e3);

        TVector3 vertex(
            phase_point.x_hcs,
            phase_point.y_hcs, 
            Get_foil_z(phase_point.foil_num)
        ); 
        auto p = phase_point.Pe_boosted; 
